...
2. [ ] 环境变量（当前先完成 dev；prod 后续再做）：
   ...
   - [ ] `prod`：以上变量（后续再做）
3. [ ] 存储适配（实现阶段）：
   - [ ] `ref` 语义升级为 Supabase object key
   - [ ] 上传：写入 Storage，返回 `{ ref, url(signed), mediaType, size, name }`
   - [ ] 回放：通过 `ref` 生成短签名 URL
   - [ ] prompt hydration：通过 `ref` 下载 bytes（图片 bytes-first；文本文件抽取注入）
4. [ ] 上传安全校验（实现阶段）：
   - [ ] 文件大小上限（初始可沿用 5MB）
   - [ ] 类型白名单（服务端校验，基于 sniffing + 声明类型）
5. [ ] 回归验证（手动）：
